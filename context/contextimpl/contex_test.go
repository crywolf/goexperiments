...
func TestBackgroundIsNotTODO(t *testing.T) {
	...
	if background == todo {
		t.Errorf("Background and TODO are equal: %q vs. %q", background, todo)
	}
}
...
func TestWithCancel(t *testing.T) {
	...
	if err := ctx.Err(); err != ErrCanceled {
...
func TestWithCancelPropagation(t *testing.T) {
	...
	if err := ctxB.Err(); err != ErrCanceled {
		t.Errorf("error should be canceled now, got %v", err)
	}
}
...
func TestWithDeadline(t *testing.T) {
	...
	if err := ctx.Err(); err != ErrDeadlineExceeded {
		t.Errorf("error should be ErrDeadlineExceeded, got %v", err)
	}

	cancel()
	if err := ctx.Err(); err != ErrDeadlineExceeded {
		t.Errorf("error should still be ErrDeadlineExceeded, got %v", err)
	}
}
...
func TestDeadlineExceededIsTimeouter(t *testing.T) {
	...
	if _, ok := err.(timeouter); ok {
		t.Errorf("canceled context error should not have Timeout method")
	}
	...
	err = f(ctx)
	if _, ok := err.(timeouter); !ok {
		t.Errorf("deadline exceeded error context should have Timeout method")
	}
}
